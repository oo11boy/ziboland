...
const EditProductPage = () => {
    ...
    fetch(`${API}/products/${id}`)
      ...
      .then((data: Product) => {
        setFormData({
          ...
          title: data.title,
          image: data.image,
          originalPrice: formatNumber(data.originalPrice),
          discountedPrice: formatNumber(data.discountedPrice),
          wholesalePrice: formatNumber(data.wholesalePrice),
          discountwholesalePrice: formatNumber(data.discountwholesalePrice),
          minwholesale: data.minwholesale.toString(),
          ...
          infotable: data.infotable
            ? data.infotable.map((item) => ({ name: item.name, value: item.value }))
            : [],
          media: data.media
            ? data.media.map((item) => ({
                type: item.type,
          ...
          hasWholesaleDiscount: data.discountwholesale !== "0",
        });
        setLoading(false);
      })
      .catch((err) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(`خطا در بروزرسانی محصول: ${(err as Error).message}`);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="container mx-auto p-4 yekan">
      <Card className="bg-white dark:bg-gray-800 shadow-lg">
        <CardHeader className="pb-3">
          <CardTitle className="text-2xl font-bold text-center">ویرایش محصول</CardTitle>
        </CardHeader>
        <CardContent>
                    ...
                    <div className="space-y-2 md:col-span-2">
                      ...
                      <div className="flex gap-2">
                        <Input
                          ...
                          onChange={(e) =>
                            setFormData({ ...formData, discountwholesale: e.target.value })
                          }
                          required
                          min="0"
                          max="100"
                ...
                <div className="p-4 space-y-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    >
                      <SelectTrigger
                        id="mothercatId"
                        className={errors.mothercatId ? "border-red-500" : ""}
                      >
