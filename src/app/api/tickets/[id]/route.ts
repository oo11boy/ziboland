...
export async function GET(
  ...
  try {
    ...
    try {
      ...
      if (!decoded.userId) {
        console.log(
          `GET /api/tickets/${ticketId}: Invalid token: User ID missing`,
        );
        return NextResponse.json(
      ...
      console.error(
        `GET /api/tickets/${ticketId}: JWT verification error:`,
        err,
      );
      return NextResponse.json(
    ...
    console.error(
      `GET /api/tickets/${params.id}: Error fetching ticket:`,
      error,
    );
    return NextResponse.json(
...
export async function PUT(
  ...
  try {
    ...
    try {
      ...
      if (!decoded.userId) {
        console.log(
          `PUT /api/tickets/${ticketId}: Invalid token: User ID missing`,
        );
        return NextResponse.json(
          { error: "Invalid token: User ID missing" },
          { status: 401 },
      ...
      console.error(
        `PUT /api/tickets/${ticketId}: JWT verification error:`,
        err,
      );
      return NextResponse.json(
        { error: "Invalid or expired token" },
    ...
    if (decoded.role !== "admin" && (response || user_id)) {
      console.log(
        `PUT /api/tickets/${ticketId}: Unauthorized attempt to modify response or user_id`,
      );
      return NextResponse.json(
    ...
    if ((result as any).affectedRows === 0) {
      console.log(
        `PUT /api/tickets/${ticketId}: Ticket not found or unauthorized`,
      );
      return NextResponse.json(
    ...
    if (status) {
      ...
      console.log(
        `PUT /api/tickets/${ticketId}: Notification created for status change`,
      );
    }
    ...
    console.error(
      `PUT /api/tickets/${params.id}: Error updating ticket:`,
      error.message,
    );
    return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    ...
    try {
      ...
      if (!decoded.userId) {
        console.log(
          `DELETE /api/tickets/${ticketId}: Invalid token: User ID missing`,
        );
        return NextResponse.json(
          { error: "Invalid token: User ID missing" },
          { status: 401 },
      ...
      console.error(
        `DELETE /api/tickets/${ticketId}: JWT verification error:`,
        err,
      );
      return NextResponse.json(
        { error: "Invalid or expired token" },
    ...
    if ((result as any).affectedRows === 0) {
      console.log(
        `DELETE /api/tickets/${ticketId}: Ticket not found or unauthorized`,
      );
      return NextResponse.json(
        { error: "Ticket not found or unauthorized" },
        { status: 404 },
    ...
    console.error(
      `DELETE /api/tickets/${params.id}: Error deleting ticket:`,
      error,
    );
    return NextResponse.json(
