...
export async function GET(request: Request) {
  try {
    ...
    const [rows] = await pool.query('SELECT username, email, phone_number, first_name, last_name, role, is_active FROM users WHERE id = ?', [userId]);
...
export async function PUT(request: Request) {
  try {
    ...
    return NextResponse.json({ message: 'اطلاعات کاربر با موفقیت به‌روزرسانی شد' });
  } catch (error) {
    console.error('Error updating user:', error);
    return NextResponse.json({ error: 'خطا در به‌روزرسانی اطلاعات کاربر' }, { status: 500 });
  }
}