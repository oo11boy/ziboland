...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { product_id, parent_id, name, rating, text, status = 0, is_admin = 0 } = await request.json();
    ...
    const [result] = await conn.query(
      ...
      [product_id, name || 'ادمین', rating || null, text, status, parent_id || null, is_admin, is_admin ? 'admin' : 'user']
    );
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to update comment', details: (error as Error).message }, { status: 500 });
  } finally {
    conn.release();
  }
}
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    const deleteReplies = async (commentId: number) => {
      const [replies] = await conn.query<RowDataPacket[]>('SELECT id FROM comments WHERE parent_id = ?', [commentId]);
    ...
    return NextResponse.json({ error: 'Failed to delete comment', details: (error as Error).message }, { status: 500 });
  } finally {
    conn.release();
  }
}