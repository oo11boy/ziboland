...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json(
      { error: "احراز هویت ناموفق یا خطا در دریافت آدرس‌ها" },
      { status: 401 },
    );
  }
}
export async function POST(request: Request) {
  try {
    ...
    if (!/^\d{11}$/.test(phone_number)) {
      return NextResponse.json(
        { error: "شماره تلفن باید 11 رقم باشد" },
        { status: 400 },
      );
    }
    // ۳. اعتبارسنجی کدپستی فقط در صورت وجود (اختیاری)
    if (postal_code && !/^\d{10}$/.test(postal_code)) {
      return NextResponse.json(
        { error: "کدپستی وارد شده باید 10 رقم باشد" },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json(
      { id: (result as any).insertId, message: "آدرس با موفقیت اضافه شد" },
      { status: 201 },
    );
  } catch (error) {
