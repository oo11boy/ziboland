...
export async function GET() {
  try {
    const [rows] = await pool.query("SELECT * FROM sliders ORDER BY slide_order");
    return NextResponse.json(rows);
  } catch (error) {
    return NextResponse.json({ error: "Failed to fetch sliders"+error}, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ error: "Failed to add slider"+error }, { status: 500 });
  }
}