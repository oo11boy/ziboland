...
const Header = () => {
  ...
  const markAllAsRead = async () => {
    ...
    if (unreadNotifications.length === 0) return toast.success('هیچ اعلان خوانده‌نشده‌ای وجود ندارد')
  ...
  const getNotificationColor = (type: Notification['type']) => {
    switch (type) {
      ...
      case 'ticket': return 'text-purple-600 dark:text-purple-400'
  ...
  const translateType = (type: Notification['type']) => {
    switch (type) {
      case 'comment': return 'نظرات'
      case 'ticket': return 'تیکت'
      case 'order': return 'سفارشات'
    ...
    <header className="bg-white dark:bg-gray-800 shadow-md border-b border-gray-200 dark:border-gray-700 px-4 sm:px-6 py-3 flex items-center justify-between sticky top-0 z-30">
      ...
      <div className="flex items-center space-x-2 sm:space-x-4">
        <DropdownMenu>
          ...
          <DropdownMenuContent style={{direction:"rtl"}} className=" w-96 max-h-96 overflow-y-auto bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700">
            <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
              <span className="text-sm font-semibold text-gray-800 dark:text-gray-200">اعلان‌ها</span>
              {unreadCount > 0 && (
                ...
                >
                  <div className="flex items-start space-x-3 space-x-reverse text-right" dir="rtl">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className={`text-sm ${notification.read ? 'text-gray-600 dark:text-gray-300' : 'text-gray-800 dark:text-gray-200'}`}>
